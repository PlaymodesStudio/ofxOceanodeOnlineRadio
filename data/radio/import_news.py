...
def download_news_stations(base_url):
    """Descarrega la llista d'estacions de ràdio categorizades com a 'news'."""
    try:
        headers = {
        ...
        # Buscar estacions amb la etiqueta 'news'
        url = f"https://{base_url}/json/stations/bytag/news"
        ...
        print(f"S'han descarregat {len(stations)} estacions de notícies.")
        return stations
    except Exception as e:
        print(f"Error descarregant les estacions de notícies: {e}")
        return None
...
def import_news_stations(stations_file):
    """Importa les estacions de ràdio de notícies i les desa al fitxer JSON."""
    ...
    # Descarregar les estacions de notícies
    stations = download_news_stations(base_url)
    if not stations:
        print("Ha fallat la descàrrega de les estacions de notícies.")
        return
    ...
    new_stations = {}
    print(f"Processant {len(stations)} estacions de notícies...")
    ...
    print(f"S'han afegit {len(new_stations)} noves estacions de notícies.")
    print(f"Total d'estacions de notícies: {len(merged_stations)}")
    return merged_stations

def main():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    stations_file = os.path.join(script_dir, 'news.json')
    
    print("Importador d'Estacions de Ràdio de Notícies")
    print("Aquest script descarregarà i importarà estacions de ràdio de notícies populars.")
    print("Les estacions existents seran preservades.")
    
    input("\nPrem Enter per continuar...")
    
    stations = import_news_stations(stations_file)
    
    if stations:
        print("\nEstacions de Notícies Importades:")
        print(f"Total estacions de notícies: {len(stations)}")
